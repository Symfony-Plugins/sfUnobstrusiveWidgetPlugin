 ...
 * jQuery UI spinner.
 *
 * Based on Brant Burnett work <http://github.com/btburnett3/jquery.ui.spinner/>.
 *
...
;(function($)
{
  ...
  $.widget('uo.uoSpinner', {
    ...
    _init: function()
    {
      ...
      {
        alert('Invalid target for uo.uoSpinner');
        return;
      }
    ...
    _createButtons: function(input)
    {
      ...
      var
        ...
        btnContainer  = self.btnContainer = $(
                                              ...
                                              '</div>'),
        rtl           = input[0].dir == 'rtl', // used to reverse left/right key directions
        upButton, downButton, buttons, icons, // object shortcuts
        hoverDelay,
        hoverDelayCallback,
      ...
      wrapper.append(btnContainer.css({
        ...
        top:    (input.offset().top - wrapper.offset().top) + 'px' // use offset calculation to fix vertical position in Firefox
      }));
            ...
            function()
            {
              setHoverDelay(
                function()
                {
                  ...
                  {
                    // ignore focus flag if show on hover only
                    self.showButtons();
                  }
                }
              );
      ...
      input
        .bind(
          ...
          function(e)
          {
            ...
            if (inKeyDown)
            {
              return false; // only one direction at a time, and suppress invalid keys
            }
            ...
            if (dir)
            {
              // only process if dir was set above
        ...
        .bind(
          ...
          function()
          {
            ...
            {
              // hovered will only be set if hover affects show
              self.showButtons();
            }
          }
        )
      ...
      function isSpecialKey(keyCode)
      {
        ...
        {
          // predefined list of special keys
    ...
    _procOptions: function(init)
    {
      ...
      if (init)
      {
        self.inputMaxLength = input[0].maxLength;
      }
    ...
    _setValue: function(value)
    {
      ...
      self.element.val(value != null ? self.options.format(value, self.places, self.element) : '').change();
    ...
    disable: function()
    {
      this.buttons
        .addClass(disabled)
        .removeClass(hover); // in case hover class got left on
